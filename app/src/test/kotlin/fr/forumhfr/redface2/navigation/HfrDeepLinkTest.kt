...
class HfrDeepLinkTest {
    ...
    @Test
    fun `user modo php with cat post numreponse lands on a TopicRoute with moderationAlertFor`() {
        val uri = Uri.parse(
            "https://forum.hardware.fr/user/modo.php?config=hfr.inc&cat=23&post=35421" +
                "&numreponse=2800456&page=76&ref=18",
        )
        val expected = ParsedDeepLink(
            destination = TopLevelDestination.Flags,
            route = TopicRoute(
                cat = 23, post = 35421, page = 76, scrollTo = 2_800_456,
                resolveScrollToPage = false, moderationAlertFor = 2_800_456,
            ),
        )

        assertEquals(expected, parseHfrDeepLink(uri))
        assertEquals(HfrDeepLinkResolution.Route(expected), resolveHfrUri(uri))
        assertEquals(
            HfrDeepLinkResolution.Route(expected),
            resolveHfrDeepLink(Intent(Intent.ACTION_VIEW, uri)),
        )
    }

    @Test
    fun `user modo php without page resolves the page (#750 rule)`() {
        val uri = Uri.parse("http://forum.hardware.fr/user/modo.php?cat=23&post=35421&numreponse=2800456")

        assertEquals(
            TopicRoute(
                cat = 23, post = 35421, scrollTo = 2_800_456,
                resolveScrollToPage = true, moderationAlertFor = 2_800_456,
            ),
            parseHfrDeepLink(uri)?.route,
        )
    }

    @Test
    fun `user modo php without numreponse falls back to the browser`() {
        val uri = Uri.parse("https://forum.hardware.fr/user/modo.php?cat=23&post=35421&page=76")

        assertNull(parseHfrDeepLink(uri))
        assertEquals(HfrDeepLinkResolution.BrowserFallback(uri), resolveHfrUri(uri))
        assertEquals(
    ...
    @Test
    fun `user modo php ignores ref and config`() {
        val base = "https://forum.hardware.fr/user/modo.php?cat=23&post=35421&numreponse=2800456&page=76"

        assertEquals(
            parseHfrDeepLink(Uri.parse(base)),
            parseHfrDeepLink(Uri.parse("$base&ref=18&config=ignored&hash_check=untrusted#t999")),
        )
    }

    @Test
    fun `user modo php requires positive integer cat post and numreponse`() {
        val valid = mapOf("cat" to "23", "post" to "35421", "numreponse" to "2800456")
        for (key in valid.keys) {
            for (invalid in listOf(null, "", "0", "-1", "abc", "2147483648")) {
                val query = (valid + (key to invalid)).filterValues { it != null }
                    .entries.joinToString("&") { (name, value) -> "$name=$value" }
                val uri = Uri.parse("https://forum.hardware.fr/user/modo.php?$query")

                assertNull(uri.toString(), parseHfrDeepLink(uri))
                assertEquals(HfrDeepLinkResolution.BrowserFallback(uri), resolveHfrUri(uri))
            }
        }
    }
    ...
    @Test
    fun `user modo php clamps invalid pages to page one and resolves the target`() {
        for (page in listOf("1", "0", "-5", "abc", "2147483648")) {
            val uri = Uri.parse(
                "https://forum.hardware.fr/user/modo.php?cat=23&post=35421&numreponse=2800456&page=$page",
            )

            assertEquals(
                TopicRoute(
                    cat = 23, post = 35421, page = 1, scrollTo = 2_800_456,
                    resolveScrollToPage = true, moderationAlertFor = 2_800_456,
                ),
                parseHfrDeepLink(uri)?.route,
            )

...
- Navigation : `CategoryRoute` pousse `TopicFormRoute(mode = New, cat, subcat)`. La capture live #214 prouve que HFR ne renvoie pas d'id sur un create réussi ; `NewTopicCreated` revient donc sur `CategoryRoute(cat, subcat = subcat-final, page = 1, highlightTitle = sujet-posté)` pour refresh la liste et mettre en évidence le sujet créé. La branche `newTopicId != null` reste défensive si HFR changeait un jour son refresh.

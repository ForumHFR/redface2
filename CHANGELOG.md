...
Phase 2 finish (#206) — create-topic : navigation directe vers le sujet créé. Phase 2G-B (#150 suite) — recherche HFR alignée sur le formulaire réel : choix Titres+messages/Titres/Messages, parsing du lien « Dernier message correspondant » quand HFR fournit un `numreponse`, pivot catégories rendu comme un scope secondaire. Phase 2G-A (#150 partiel) — recherche HFR réelle dans les titres de topics (`forum1.php?recherches=1&...`), parser pivot/single/multi/no-results, écran Recherche fonctionnel avec navigation vers le topic. Phase 2F-C (#11 partiel) — picker smileys symétrique sur `TopicFormScreen` (Edit FP + New topic). Phase 2F-B (#11 partiel) — picker smileys dans l'éditeur (bottom-sheet Material 3, onglet Standard 25 builtins HFR + onglet Wiki live via `message-smi-mp-aj.php`). Phase 2E (#149) création de topic + follow-up Phase 2B-B (#144) déjà mergés décrits plus bas.

### Added (Phase 2 finish #206)
- `ReplySubmitResponseParser` extrait le `topicId` (1er entier du segment `sujet_{topicId}_{page}` de l'URL `<meta refresh>`) en plus de la page et du numreponse, via un `SUJET_SEGMENT_REGEX` ancré (`(?<![a-z_])sujet_(\d+)_(\d+)`). Nouveau champ `ReplySubmitResult.Success.topicId: Int?`.
- `DefaultTopicFormRepository.submitNewTopic` branche `(newTopicId, newNumreponse)` réels dans `NewTopicSubmitResult.Success` au lieu de `(null, null)` ; `RedfaceNavigation.onNewTopicCreated` propage `submitSignal` pour ouvrir directement le sujet créé au 1er post.
- Contrat documenté dans `protocol-hfr.md` (shape `bddpost.php` partagée reply/quote/create, validée sur fixtures réelles `write_reply_success_response` / `write_quote_success_response` / `write_edit_first_post_success_response`). Fallback `null` → `CategoryRoute` couvert par un test repo dédié.
...
- Écran Recherche : chips de scope textuel, pivot catégories horizontal avec texte d'aide, lieu du résultat (`cat/subcat`) dans chaque carte, extrait du message correspondant quand disponible, placeholder sur les filtres auteur/date/pagination à venir.

### Fixed (Phase 2G-B #150 suite)
...
- `HfrClient.searchTopics(...)` : endpoint **anonymous** vers `/forum1.php?recherches=1&...`, paramètres exhaustifs (`titre`, `orderSearch`, `daterange=2`, etc.) avec date injectée pour testabilité, pas de cookies auth.
...
- Navigation : `SearchRoute` câblé sur `TopicRoute(cat, post, page, scrollTo)` ; les résultats titre ouvrent page 1, les résultats contenu utilisent le `numreponse` si HFR le fournit.
...
- `TopicFormIntent` ajoute `SmileyPickerOpened`, `SmileyPickerDismissed`, `SmileySearchQueryChanged(query)`, `SmileySelected(token)`.
- `TopicFormViewModel` injecte `SmileyRepository`, applique le même debounce 300 ms / gate `query.length > 2` / identity guard que `PostEditorViewModel`, hydrate `userId` depuis `TopicForm.userId` (anti-clobber sur refetch silencieux).
- `TopicFormScreen` affiche le bouton « Smileys » à côté de la toolbar BBCode et monte `SmileyPickerSheet` en sibling du contenu scrollable, pour les deux modes (`Edit FP`, `New topic`).

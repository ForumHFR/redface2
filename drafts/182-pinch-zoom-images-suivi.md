...
| `PINCHING` | Le 2e pointeur posé ANNULE tout swipe non commité ; scale autour du centroïde ; plancher 1×, plafond 2,5× (rubber-band au-delà) |
| `ZOOMÉ` | 1 doigt : pan X borné dans la layer (translationX ∈ [width×(1−scale), 0]) ; pan Y = scroll RÉEL de la liste (jamais de translationY persistante — la virtualisation interdit de révéler des items non composés) ; swipe/PTR/double-tap/sélection/scroll-`[fixed]` suspendus ; **taps et long-press image : contrat = CONSERVÉS** (résultat attendu ; si le POC montre des déclenchements parasites pendant le pan → repli acté : désactivés à >1× comme la sélection, et ce draft mis à jour) |

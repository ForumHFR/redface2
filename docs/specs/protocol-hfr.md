...
- **Tailles différentes** : les smileys ne sont pas tous 16×16 — corpus perso échantillonné live (~25 GIFs `images/perso/*.gif`) : majorité 50×50 à 70×50, fraction <30 px (`tinostar` 15×15, `grilled` 15×20, `aulas:3` 27×24), rares grands formats. Phase 1 utilise une politique de buckets (`:core:ui` `PostMediaDisplayPolicy`) : builtin **bucket fixe 18×18**, perso **bucket fixe 56×56**. Les smileys utilisent `ContentScale.Fit` pour rester lisibles sur smartphone : un sprite 15×15 est agrandi à `56×56`, un 39×15 devient `56×22`, un 70×50 devient `56×40`, ratio préservé. Les images inline `[img]` gardent `ContentScale.Inside` pour éviter d'agrandir une petite image arbitraire dans le bucket `240×180`. L'enfant `AsyncImage` du `Placeholder` utilise `Modifier.fillMaxSize()` au lieu d'un `Modifier.size(.dp)` figé — l'image suit ainsi le placeholder en `sp` même quand `fontScale ≠ 1` (accessibility). `PlaceholderVerticalAlign.Center`. Invariant pinned dans les tests : `persoSmiley.placeholderHeight ≤ 2.8 × bodyMedium.lineHeight` (lecture dynamique sur `RedfaceTypography`, `20.sp` aujourd'hui). Mesure intrinsèque via `ImageLoader.execute()` reportée Phase 2/4 si le bucket fixe reste insuffisant.

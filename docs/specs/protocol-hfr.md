...
- **Tailles différentes** : les smileys ne sont pas tous 16×16 — perso peuvent atteindre 200×150. Phase 1 utilise une politique de buckets (`:core:ui` `PostMediaDisplayPolicy`) : builtin 18×18, perso 64×64 max avec `ContentScale.Fit` (downscale, jamais squash). Mesure intrinsèque via `ImageLoader.execute()` reportée Phase 2/4 si le squash perso devient gênant en pratique.

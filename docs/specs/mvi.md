...
Phase 2B-A (#86 + #144) a séparé l'éditeur en **deux familles** plutôt qu'en un mode unique. Phase 2C ajoute les deux mutations HFR principales : le submit reply (#145, via `bddpost.php`) et la quote MVP (#146, même endpoint POST mais GET form via `numrep={cited}&ref={N}`). Phase 2D (#147) ajoute l'édition d'un post existant : `PostEditorMode.Edit` GET `message.php?…&numreponse={N}`, hydrate le draft depuis le BBCode existant, et POSTe `bdd.php?config=hfr.inc`. Edit FP (#148) et création de topic (#149) restent en attente.
...
> **Statut Phase 2C+2D — Reply + Quote + Edit post MVP livrés** : `PostEditorViewModel` injecte `ReplyRepository` (Phase 2C) **et** `EditPostRepository` (Phase 2D #147), tous deux dans `:core:domain/write/` (impl dans `:core:data/write/`). Sur init en mode Reply, il fetch `message.php` pour obtenir `hash_check` + les hidden fields HFR ; quand `quotedNumreponse != null`, le GET passe `numrep={cited}&ref={N}` et HFR renvoie un `<textarea name=content_form>` prérempli avec un bloc `[quotemsg=…]`. En mode Edit, le GET passe `numreponse={N}` à la place et HFR renvoie le BBCode existant du post. `ReplyForm.initialContent` est hydraté dans `draft.text` au premier load et **jamais** réécrit ensuite (`draftHydratedFromForm` mémorise l'opération pour qu'un refetch silencieux sur `InvalidHashCheck` n'écrase pas l'édition en cours). Sur `SubmitClicked` POSTe `bddpost.php?config=hfr.inc` (reply/quote) ou `bdd.php?config=hfr.inc` (edit) avec respectivement `numrep={cited}` / `numreponse={edited}`. Anti-double-submit via `submitJob.isActive`. Errors typées via `ReplyFailureReason` (empty / invalid_token / antiflood / locked / login_required / unknown) + `SubmitError.Network` / `SessionExpired` / `MissingSubcat`. Sur succès, effet `SubmitSucceeded(targetPage, scrollTo?)` → la navigation pop l'éditeur et recharge la page topic ; pour edit, `scrollTo = numreponse` cible le post édité après refresh. Edit FP / Create restent dans #148 / #149. `TopicFormScreen` reste placeholder.
>

...
- [x] **Recherche HFR titres MVP (#150 partiel, Phase 2G-A)** — recherche dans les **titres de sujets** via `GET /forum1.php?recherches=1&...` (le form HFR `POST /search.php` renvoie une page de transition meta-refresh, on hit le GET canonique directement). Parser couvre 4 shapes capturées : `no-results` (page `.hop` minimaliste), `pivot single` (1 cat hit), `pivot multi` (N cats hit, sélecteur via `<select name="cat">` dans `<div class="search">`), `explicit cat` (listing standard sans bannière). Écran Recherche : champ + IME Search + bouton, états idle/loading/empty/error/results, AssistChip de pivot pour relancer dans une autre catégorie. Navigation → `TopicRoute(cat, post, page=1)` puisque les résultats titres ne portent pas de `numreponse`. Recherche dans le contenu des posts + filtres auteur/date reportés en `2G-B` (nécessite fixtures dédiées).
- [ ] Recherche HFR contenu posts + filtres auteur/date (#150 suite, Phase 2G-B)

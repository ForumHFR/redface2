...
- [x] MultiMPs — liste avec vue drapeaux, lecture, reply, onglet « DT » des Drapeaux listant les MultiMP + reprise de lecture MPStorage livré 2026-06-19, app-v166. La citation simple par message (#1074) y est câblée comme sur les MP classiques, mais son contrat n'est mesuré qu'en 1:1 : les formulaires de réponse DT capturés (owner comme participant) portent un `ref=0` caché et aucun champ `numreponse`, que la validation de citation rejette — en DT l'action est masquée ou échoue fermé tant que le formulaire de citation DT n'a pas été capturé ; citation multiple absente

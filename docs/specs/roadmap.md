...
- [ ] **Edit FP polish (#178)** — finaliser le cas de recatégorisation depuis le dropdown sous-catégorie et vérifier que l'UX ne suggère pas de support sondage actif tant que les fixtures POST sondage manquent.
...
- [ ] **Médias éditeur Phase 2F-E (#189)** — helper MVP d'insertion d'image par URL (`[img]url[/img]`). Upload/rehost, favoris/récents smileys, GIFs externes et sync MPStorage sont différés aux phases ultérieures (#190).
...
- [ ] **Recherche HFR bugfix (#188, Phase 2G-B)** — corriger l'affichage des résultats catégorie quand la page de résultats renvoie un pivot/sélecteur et éviter un clic intermédiaire inutile avant d'ouvrir le topic.
...
- [ ] **Réglage proxy utilisateur (#187, Phase 2H)** — exposer un réglage proxy HTTP/HTTPS avec authentification optionnelle, persistance DataStore, branchement OkHttp + Coil, diagnostics sans fuite de credentials. Le MVP peut demander un redémarrage de l'app après changement ; pas de proxy embarqué, PAC ou bypass list.

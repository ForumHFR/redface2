...
- [x] **Smileys éditeur MVP (#11 partiel, Phase 2F-B)** — picker `ModalBottomSheet` Material 3 dans `PostEditorScreen` (Reply / Quote / Edit). Onglet **Standard** : 25 builtins HFR servis depuis la constante `BUILTIN_HFR_SMILEYS` (codes dérivés de `write_reply_form_open_topic.html`). Onglet **Wiki** : recherche live via `GET /message-smi-mp-aj.php?config=hfr.inc&user_id={id}&findsmilies={query}` (contrat vérifié 2026-05-22, `user_id` parsé depuis `find_smilies_timer('hfr.inc', N)` du form HTML et plumb dans `ReplyForm.userId`/`TopicForm.userId`), debounce 300 ms + gate `query.length > 2` alignés sur le composer HFR. Insertion via `insertBbcodeToken(" $token ")` qui reproduit la convention `putSmiley` JS. Hors scope #11 conservé : favoris, récents, upload smiley perso, GIFs externes, catalogue offline, picker dans `TopicFormScreen` (à câbler dans une PR suivante).

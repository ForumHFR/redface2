...
- [x] MultiMPs — liste avec vue drapeaux, lecture, reply, onglet « DT » des Drapeaux listant les MultiMP + reprise de lecture MPStorage livré 2026-06-19, app-v166. La citation simple par message (#1074) est livrée comme sur les MP classiques et son GET de formulaire a été mesuré en DT le 2026-08-17 : mêmes 20 champs cachés que le 1:1, `cat=prive`, `numreponse` vide, `numrep` = message cité, aucun `ref` caché et aucun `newdest`. Le `ref=0` sans `numreponse` appartient au formulaire de **réponse** DT owner et ne décrit pas la citation. Vérifié aussi sur appareil : « Citer » ouvre l'éditeur prérempli par HFR sans erreur ; aucun POST live n'a été émis. Citation multiple absente

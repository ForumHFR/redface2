...
| Contenu des conversations MP | RAM d'abord (`PrivateMessageThreadSessionCache`, LRU de cinq pages), puis Room si l'opt-in global est ON (`mp_thread_pages` + `mp_messages`, cinq pages par compte), puis réseau obligatoire. Les hits cache sont provisoires ; seul le réseau visible terminal, cible et sceau revérifiés, alimente RAM et Room. Le prefetch reste RAM-only. Purge au logout / changement de compte et purge globale au passage OFF. | RAM : session processus ; Room : jusqu'à éviction, déconnexion ou OFF |
...
> **Acté — [ADR-018]({{ site.baseurl }}/adr/018-mp-cache-disque-opt-in) (accepté 2026-08-25, supersède l'ADR-013)** : politique de cache des conversations MP à trois étages — position de lecture locale par conversation (survit au process death, purgée à la déconnexion), cache RAM de session, cache Room du contenu en opt-in explicite (défaut OFF, purge au logout). **État réel** : les trois substrats sont livrés **et l'étage Room est exposé** depuis la PR 3 du lot 7 ([#1097](https://github.com/ForumHFR/redface2/issues/1097)) — un toggle unique, global à l'application et à tous ses comptes, défaut OFF. La position locale existe depuis la Phase 3 ([#430](https://github.com/ForumHFR/redface2/issues/430)) via `MpReadPositionEntity`, `RoomPrivateMessageReadPositionStore` et la table `mp_read_positions`. Le contenu passe d'abord par `PrivateMessageThreadSessionCache`, puis par les tables séparées `mp_thread_pages`/`mp_messages` seulement si l'opt-in global est effectivement ON, sans jamais supprimer la revalidation réseau. Un verrou singleton commun ordonne chaque accès avec OFF et les purges : `CacheInvalidator` avance la génération RAM avant celles-ci, une transaction déjà admise est purgée ensuite, et une tentative plus tardive voit OFF ou un sceau périmé. Une purge échouée reste marquée, désactive effectivement l'accès et est retentée au démarrage suivant — **seule** une purge pendante déclenche un scrub au lancement, un défaut OFF sans purge en attente n'en déclenche aucun. Les trois événements de confidentialité (passage à OFF, déconnexion, bascule de compte) finissent par un scrub des octets dans `PrivateContentDatabaseScrubber` : `PRAGMA wal_checkpoint(TRUNCATE)`, `VACUUM`, puis un **second** checkpoint TRUNCATE — en WAL le `VACUUM` commite son image propre dans le journal, donc sans ce second checkpoint le fichier `redface.db` conserverait les octets purgés. Depuis #1096, les médias issus de ce contenu restent non persistants par défaut : leurs requêtes Coil désactivent le cache disque et une transition de compte purge le cache global historique, faute de marque permettant de distinguer les anciennes entrées MP des images publiques.
...
- purge les pages et messages MP de l'ancien compte par la façade dédiée. La clé Room applique la
  convention historique `lowercase()` à l'écriture comme à la purge ; aucun identifiant de
  conversation, page ou compte n'est journalisé en cas d'échec.
...
Une **partie** des règles d'architecture décrites plus haut est **enforcée mécaniquement** par **[Konsist](https://docs.konsist.lemonappdev.com/)** (Kotlin-first, AST parsing) — pas par une simple convention markdown. Couverture actuelle : **11 cas `@Test`** dans `ArchitectureKonsistTest`, soit **8 règles d'architecture** et **3 auto-tests des gardes**. Les 8 règles : features → pas de couche d'implémentation `:core:*` (`data`/`network`/`parser`/`database`, slice `/feature/`) ; `:core:extension` limité à `topic`/`editor` ; tokens M3 instanciés hors `:core:ui` ; code authentifié-only (`/auth/`, `/messages/`) sans `@AnonymousClient` ; `PrivateMessageThreadSessionCache` memory-only et silencieux (#1080/#316) ; accès aux tables et au DAO de contenu MP réservé aux déclarations Room, au wiring et à la façade disque silencieuse ; `:feature:topic` → pas de `:feature:profile` (#208) ; sites d'appel de prefetch limités aux points d'entrée dédiés, avec l'allowlist du prefetch MP authentifié (ADR-018 décision 7). Les 3 contre-épreuves vérifient les formes réellement reconnues par ces scans, notamment les contournements par accesseur de base, noms de table et diagnostics. **Non couverts à ce jour** : les dépendances autorisées **inter-`core`** hors cette frontière MP explicite, les autres frontières propres au module `:app`, et l'instanciation M3 via fabriques (`lightColorScheme()`) ou usage pleinement qualifié. À étendre (cf. charte méthodo) — la prose ne doit pas affirmer plus que ce que Konsist vérifie réellement.

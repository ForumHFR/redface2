...
> **Note Phase 1B.4 — `:feature:flags` livré** : l'écran d'accueil (Drapeaux) vit dans `:feature:flags` avec `FlagsViewModel` (Hilt) + `FlagRepository` + 3 onglets (`FlagType.CYAN` = mes sujets, `RED` = lus uniquement, `FAVORITE`). `:app` ne fait que la navigation (`FlagsRoute(onOpenFlag, onLoginRequested)` après le polish #154). Le footer alpha (pseudo, logout, version, signalement, Diagnostics) qui vivait initialement dans `FlagsRoute` a été déplacé sur `MessagesScreen(versionName, versionCode, onLoginRequested, onOpenDiagnostics)` jusqu'à ce que Phase 3 livre la vraie liste de MPs ; `:app` y injecte toujours `BuildConfig.VERSION_NAME/VERSION_CODE` pour afficher « Redface 2 — vX.Y (build N) ».

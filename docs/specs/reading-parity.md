...
| Ancres de scroll par page (session) | #307/#895 F3, `pageAnchors` | **oui mais absent** | Le topic restaure la position par page visitée (moteur in-VM) ; le MP atterrit en haut à chaque changement de page. Depuis #1093, cette remise en haut n'est plus portée par `ScrollToTopOnPageChange` mais par `PrivateMessagePageLandingEffect` (`:feature:messages`), **autorité unique d'atterrissage** de la liste MP : elle arbitre entre remise en haut et atterrissage sur le message cité, et le second gagne. Le composable partagé de `:core:ui` n'a plus aucun site d'appel de production (son seul appelant restant est son propre test) — code mort suivi hors de cette page par [#1104](https://github.com/ForumHFR/redface2/issues/1104) (retrait avec son test, ou hôte explicitement justifié) : la garde `[enforced]` vérifie que les symboles cités existent, pas qu'ils soient encore montés, donc elle ne peut pas attraper cette dérive. La reprise de **page**, elle, est livrée (ligne dédiée). |
| Pagination riche (picker, premier/dernier, snapshots RAM, stale-while-switching, chrome) | #895 étape 4, `TopicViewModel.switchToPage` | **oui mais absent** | Le cache RAM prérequis est livré ; le prefetch borné est câblé mais attend sa preuve live multipage (ligne dédiée). Le picker, les raccourcis premier/dernier, les ancres par page et le chrome riche restent au lot 6 ([#1103](https://github.com/ForumHFR/redface2/issues/1103), PR 2 et PR 3). Après envoi, `onSubmitSucceeded` retire encore l'entrée `PrivateMessageThreadRoute` puis en ajoute une copie avec un `submitSignal` neuf, ce qui recrée `PrivateMessageThreadViewModel` : tout état de pagination de session porté par ce ViewModel est alors jeté, annulant l'intérêt de le conserver. Supprimer ce remplacement de route est donc un point du lot 6, pas un détail ultérieur. |

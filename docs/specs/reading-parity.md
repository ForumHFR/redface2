...
| Saut vers le message cité | #625/#699/#782/#1074, `onGoToCitedPost` | **oui mais absent** (cible parsée, geste MP non câblé) | La page et le `numreponse` cités sont désormais extraits des liens statiques et dynamiques par le `PostContentParser`, y compris sur la fixture MP en cat=prive. Le défaut de forme dynamique touchait aussi les sujets en authentifié (#625), il n'était pas propre aux MP. `onGoToCitedPost` n'est toujours pas passé au `PostRenderer` côté MP : le geste, l'atterrissage différé et l'arbitrage du retour #782 restent à livrer. |

...
| Zoom pincé | #182/#1040, `PinchZoomState` + `pinchZoom` + `pinchZoomTransform` | **oui, livré** (lot 6, PR 1) | État, geste, transformation et calculs vivent dans `:core:ui`. Topic et MP montent la même machine ; chaque feature garde sa clé page/lecteur, son swipe, son pull-to-refresh et son chip 1×. Les suites Topic inchangées épinglent calculs, gestes, repli de citation et coexistence multi-touch ; les tests MP prouvent le pincement effectif, le reset à la clé de page et la suspension du swipe/PTR. Les captures Roborazzi avant/après à 1× et à transformation fixe sont une preuve pixel du rendu extrait, pas une preuve gestuelle. |

...
| Double-tap pour rafraîchir | #382 | **oui mais absent** | Geste topic (RF1 parity), câblé au niveau de la liste dans `TopicScreen` ; le MP n'a que le pull-to-refresh — l'action (recharger la page courante) existe des deux côtés, seul le geste manque, rien de serveur. **Lot 6**, en **élargissement explicite** : la famille « gestes » du titre du lot l'accueille, mais sa cellule de cadrage ne le liste pas (et #1050 l'exclut du lot 2) — décision du 2026-08-15, désormais portée par l'issue du lot 6 ([#1103](https://github.com/ForumHFR/redface2/issues/1103), PR 5, placée en dernier pour être testée contre le zoom et le swipe définitifs). Elle en fixe les cinq critères d'acceptation, repris verbatim du contrat topic (`TopicScreen.kt`) : **(a)** les clickables enfants consomment leurs propres up events, **(b)** un drag au-delà du touch slop annule le geste, **(c)** geste suspendu pendant le zoom (`!zoomed`), **(d)** tick haptique `HapticFeedbackType.Confirm`, **(e)** feedback visuel par le spinner du `PullToRefreshBox` partagé. Piège propre au MP, également cadré là-bas : `messageMenuLongPress` monte déjà `detectTapGestures` sur chaque carte — un arbitre unique sur la surface de carte, pas de `combinedClickable` racine. |
...
| Ancres de scroll par page (session) | #307/#895 F3, `pageAnchors`, `PrivateMessagePageLanding`, `PrivateMessageListAlignment` | **oui, livré** (lot 6, PR 2) | Le MP capture index + offset à l'arrêt d'un scroll utilisateur, sous verrou d'alignement contenu/position et hors drag d'ascenseur ou geste/settle de zoom. Le ViewModel retenu conserve une ancre RAM par page visitée et arbitre un seul atterrissage : message cité explicite > ancre restaurée > haut ; le cité consomme seulement l'intention pendante, jamais l'ancre mémorisée. La première émission rendue de la cible (cache de session si présent, réseau sinon) porte l'atterrissage ; la revalidation ne rescrolle pas. **Portée session d'affichage uniquement** : aucune persistance, aucun SavedStateHandle, donc aucune restauration après mort de process — divergence assumée avec le sujet, à traiter séparément si le miroir primitif est porté plus tard. Purge complète au logout et au changement de compte ; le prefetch ne crée aucune ancre. |
| Pagination riche (picker, premier/dernier, snapshots RAM, stale-while-switching, chrome) | #895 étape 4, `TopicViewModel.switchToPage`, `PrivateMessageSubmitResult` | **oui mais absent** (livraison partielle) | Le cache RAM est livré et le prefetch borné est câblé ; les ancres par page et la session ViewModel conservée après envoi sont livrées par le lot 6 PR 2. Le succès d'envoi passe désormais par un résultat opaque en mémoire, publié avant le pop et consommé une fois par le ViewModel retenu : la route n'est plus remplacée et son champ `submitSignal` reste seulement pour compatibilité de sérialisation. Le picker, les raccourcis premier/dernier et le chrome riche restent à la PR 3 ; le prefetch attend toujours sa preuve live multipage (ligne dédiée). |

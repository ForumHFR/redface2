...
| Pinceau doré des créateurs | #221, `isRf2Creator` + `rememberCreatorPseudoBrush` | **oui mais absent** | Détection (`:core:domain`) et pinceau (`:core:ui`) déjà partagés, et #221 veut le pseudo doré « everywhere it appears » : un créateur peut être l'auteur d'un MP, la fonction s'applique — rien de serveur, la détection est une liste statique embarquée. Le topic passe son `CreatorPseudoText` par le slot `pseudo` de `PostIdentityHeader` ; `MessageCard` reste sur le repli neutre. **Porteur : issue dédiée, hors lots** (fonction isolée, sans dépendance aux lots 2–7, exclue du lot 2 par #1050 ; une issue se suit mieux qu'un lot d'un seul élément) — activation du slot `pseudo` de `PostIdentityHeader` côté MP, qui devra porter son propre `heading()` (contrat #884). Issue porteur : **#1060**. |
...
| Marqueur d'édition | #483, `Post.editedAt` | **oui mais absent** | Deux rendus topic : marqueur inline « · édité » (#483, slot `dateTrailing` de `PostIdentityHeader`, non fourni par `MessageCard`) et ligne « Édité le … » du menu (#362). Parsing partagé : `PostsParser` extrait `editedAt` aussi sur les pages `cat=prive` ; présence du trailer non caractérisée par la fixture de #1041 (aucun trailer d'édition dedans) — même doctrine que les signatures (#1050) : câblage data-driven, inerte si HFR ne sert rien, « câblé, non prouvé live » tant qu'une fixture de MP édité n'a pas tranché. **Lot 3 PR 3** (menu de message MP), en **élargissement explicite** de #1051 — l'issue constate que le profil et `editedAt` sont communs mais sa PR 3 ne les engage pas ; à porter dans #1051. |
| Compteur de citations | #239/#863, `Post.citedCount` | **oui mais absent** | Pill « cité N fois » (slot `badges` de `ReadingPostCard`, non fourni par `MessageCard`) + ligne du menu côté topic ; compteur **serveur** (#863) extrait par le `PostsParser` partagé, donc déjà parsé en MP s'il est servi. Présence de « Message cité N fois » sur les pages `cat=prive` non caractérisée par la fixture de #1041 (elle n'en contient pas) — même doctrine que les signatures (#1050) : câblage inerte sans donnée, « câblé, non prouvé live » avant fixture. **Lot 3 PR 3**, même **élargissement explicite** de #1051 que le marqueur d'édition (métadonnées communes du message : ligne du menu, pill par le slot `badges`) ; à porter dans #1051. |

...
| Pagination riche (picker, premier/dernier, snapshots RAM, stale-while-switching, chrome) | #895 étape 4, `TopicViewModel.switchToPage` | **oui mais absent** | Le cache RAM prérequis est livré ; le prefetch borné est câblé mais attend sa preuve live multipage (ligne dédiée). Le picker, les raccourcis premier/dernier, les ancres par page et le chrome riche restent au lot 6. |

...
> **Refonte #604 en cours** (vagues 1-4 livrées en dev 0.19.x→0.24.x) — skeleton de chargement (`TopicLoadingSkeleton`), header dissous (pilule « page X / Y » cliquable → feuille de navigation), cartes de frontière de page (`PageBoundaryCard`/`EndOfTopicCard`), repère « dernier message lu » traversant (#600), réponse rapide en bottom sheet (`QuickReplySheet`) avec escalade vers l'éditeur plein écran (le brouillon suit, #405), multi-quote avec seuil d'escalade. Le rendu des citations dans le composer est en arbitrage (#805 : BBCode inline par défaut, cartes en option). La navigation par pages ci-dessous (#282/#307) reste exacte.
...
**Actions sur un post :**
- **Citer** → feuille de réponse rapide pré-armée avec la citation ; escalade possible vers l'éditeur plein écran. « Citer N » (multi-quote) ouvre directement l'éditeur plein écran à partir de 3 citations (seuil épinglé par test). Rendu des citations : cf. arbitrage #805

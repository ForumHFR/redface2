...
- **Zoom pincé** (#182/#1098, lot 6 de #1040) : la loupe éphémère de la vue Topic
  est montée à l'identique dans la conversation — `PinchZoomState` +
  `Modifier.pinchZoom` / `pinchZoomTransform` de `:core:ui/zoom/`, à rendu
  identique au pixel près. Elle est **globale et éphémère** : la clé de page
  réinitialise le zoom, un chip « 1× » remet au repos, et pendant le zoom le
  défilement natif, le swipe de page et le pull-to-refresh sont **désarmés** (les
  gestes eux-mêmes, pas seulement leurs effets) ; l'axe vertical est alors piloté
  par la machine de zoom. Depuis #1106 (2026-09-02), les taps et appuis longs des
  enfants restent actifs dans la couche transformée : liens, images et sélection
  de texte répondent en Topic comme en MP ; seul un déplacement au-delà du touch
  slop engage le pan zoomé et consomme le geste.

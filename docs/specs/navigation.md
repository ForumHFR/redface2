...
- **Réponse** (#301) : le FAB global « Répondre » ouvre le composer (formulaire sourcé depuis `message.php`, fallback formulaire embarqué — cf. `DefaultPrivateMessageWriteRepository`) ; gaté sur `thread.canReply`. **Pas de citation par message** : le `numrep` prérempli est le dernier message de la page, pas une référence de citation (requalifié par #1041 ; citation MP = lot 4 de #1040, après spike du formulaire réel).

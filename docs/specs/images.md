...
**[AMENDEMENT-v1.6-11 — #1350 / #972 — §5 chargement réseau et en-têtes par hôte, §6 réponse vide ; écriture Sol 13/09, vérification live Fable 12/09]** **Chargement réseau et en-têtes par hôte (§5).** Les images sont chargées avec le client HTTP anonyme, sans cookies de session. Pour une URL dont l'hôte est exactement `reho.st` ou un sous-domaine strict de `reho.st`, la requête de chargement porte `Referer: https://forum.hardware.fr/` (reho.st renvoie 403 à toute requête sans Referer `*.hardware.fr`, vérifié live le 12/09/2026). Cet en-tête n'est envoyé à aucun autre hôte, notamment Imgur, Diberie ou IBB. Au point de chargement Coil uniquement, une URL `http://` dont l'hôte exact appartient à l'allowlist HTTPS prouvée est réécrite en `https://` ; l'allowlist initiale est `{reho.st}` (même chemin, même octets en https, sans redirection serveur). L'URL source reste inchangée dans le contenu parsé, la visionneuse et les actions d'ouverture externe. Toute autre URL `http://` reste inchangée et peut échouer sous la politique cleartext Android (statu quo #972). **Réponse vide (§6).** Un statut HTTP 200 dont le corps est vide ou non décodable comme image (reho.st sur un hash inconnu) ne constitue pas un succès de rendu : il produit un échec de décodage et suit l'état painter KO dans le slot réservé, avec les règles de retry et d'anti-CLS existantes.

**[AMENDEMENT-v1.6-12 — #1359 / #972 — §5 images `http://` chargées telles quelles ; décision XaTriX 13/09 (« lire les images http »), rédaction Fable]** **Images `http://` (§5).** Remplace, dans [AMENDEMENT-v1.6-11], la phrase « Toute autre URL `http://` reste inchangée et peut échouer sous la politique cleartext Android (statu quo #972) ». Une URL d'image `http://` est chargée **telle quelle**, en clair, par le client image anonyme, quel que soit l'hôte. La réécriture `http://reho.st` → `https://reho.st` de v1.6-11 est conservée (octets identiques vérifiés, transfert chiffré quand c'est possible) ; aucune autre réécriture ni allowlist. L'application déclare une configuration de sécurité réseau autorisant le trafic en clair (`cleartextTrafficPermitted="true"` en base-config — Android ne permet pas de le restreindre à un client HTTP, seulement par domaine). Garde-fous : les clients HFR (authentifié, mutation, upload) n'exécutent jamais de requête en clair par construction (URL de base `https://forum.hardware.fr`) et refusent toute redirection `https` → `http` (`followSslRedirects(false)`), de sorte qu'aucun cookie de session ne peut transiter en clair ; seul le client image anonyme, sans cookies, utilise cette permission. Le `Referer` reho.st de v1.6-11 est inchangé et s'applique aussi aux URL `http://reho.st`. Un hôte injoignable, un 4xx/5xx, un corps vide ou non décodable restent un échec de décodage au sens du §6 (slot KO, retry manuel). L'URL source reste intacte partout (contenu parsé, visionneuse, ouverture externe).

**[AMENDEMENT-v1.6-13 — #1367 / #1368 — §5 client image dédié, garde-fou des lectures HFR anonymes, bascule reho.st sur tous les chemins ; review de promotion #1366 (R1, R5), rédaction Fable 13/09]** **Client image dédié (§5).** Précise [AMENDEMENT-v1.6-12] : le « client image anonyme » est un client HTTP distinct des quatre clients HFR (authentifié, mutation, upload, anonyme), réservé au chargement des images (Coil) et à leur enregistrement (galerie). Lui seul suit les redirections, y compris `https` → `http`, et il ne porte aucun cookie. Le client HFR anonyme (pages lues sans session, API REST) refuse les redirections `https` → `http` comme les trois autres clients HFR : une telle redirection est une erreur dure, jamais un repli en clair ; les redirections de même schéma restent suivies (HFR répond `301` relatif sur `forum2.php`, vérifié live le 13/09/2026, aucun `https` → `http` observé sur 24 endpoints). Cette garde est prouvée par un test de comportement (un serveur de test répond `302 Location: http://…`, aucun client HFR ne suit, le client image suit) et non par la seule lecture du drapeau. La permission cleartext Android reste déclarée pour l'application entière, Android ne permettant pas de la restreindre à un client ; le texte destiné aux utilisateurs le dit ainsi : « l'application autorise le trafic en clair ; côté code, seul le chemin image l'emprunte ». **Bascule reho.st sur tous les chemins (§5).** Remplace, dans [AMENDEMENT-v1.6-11], « Au point de chargement Coil uniquement » : la réécriture `http://reho.st` → `https://reho.st` est portée par le client image lui-même (intercepteur HTTP), de sorte que l'affichage et l'enregistrement partagent la même requête, la même clé de cache disque (URL source) et le même transfert chiffré ; elle s'applique à l'hôte exact `reho.st` et à ses sous-domaines stricts, comme le `Referer` de v1.6-11. L'URL source reste inchangée dans le contenu parsé, la visionneuse et l'ouverture externe.

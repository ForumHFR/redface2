...
- **Rédaction longue : défilement interne, hauteur contrainte en paysage** (#447, #1263, #1406, `0.60.0`). Le champ repose sur `BasicTextField(TextFieldState, …)` et son `ScrollState` : le contenu défile dans le champ, le curseur reste visible pendant la frappe et les poignées suivent une sélection étendue. L'éditeur de post range désormais son titre, sa barre d'outils et l'aperçu dans la même zone défilante que les autres contrôles ; dans les quatre éditeurs, les alertes passent en tête. Dès que 220 dp restent disponibles pour la zone et le champ, 160 dp sont réservés à la saisie. Limite restante : en paysage avec le clavier ouvert et seulement ≈ 100 dp disponibles, la zone de contrôles reçoit un tiers de l'espace et le champ conserve ≈ 55 dp après l'espacement ; tous les contrôles restent atteignables en faisant défiler leur zone, et masquer le clavier rend l'espace au brouillon.

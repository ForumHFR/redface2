...
- **Pas de défilement continu pendant le glissement d'une poignée de sélection** (#447 point 2, #1263, suivi par #1406). Depuis la `0.59.1`, l'éditeur suit le bord de la sélection **réellement déplacé** (la poignée du haut comme celle du bas), avec une ligne d'avance. Ce qu'il ne fait pas : faire défiler le texte tant qu'on **maintient** une poignée contre le bord de l'écran. Limite plateforme, pas un mauvais conteneur de défilement : les poignées sont rendues dans leur propre fenêtre `Popup` (leurs événements pointeur ne traversent pas le champ) et `TextFieldSelectionManager` — le gestionnaire du `BasicTextField(TextFieldValue, …)` hérité — ne contient aucun code de défilement (vérifié sur foundation 1.11.2) : il cumule des deltas de pointeur bruts sur une origine capturée dans le repère du layout de texte, donc faire défiler un ancêtre **désynchronise** le glissement au lieu d'étendre la sélection. Les deux issues de sortie (scroller entre la decoration box et l'inner text field, ou migrer vers `BasicTextField(TextFieldState, …)`) reviennent toutes deux à redonner un défilement interne au champ, c'est-à-dire à rouvrir le contrat de scroll externe posé par #422/#434.

...
- **Bêta publique 0.55.0** (7 septembre 2026, Play test ouvert + F-Droid) : visionneuse d'images plein écran (pincement, partage, tap pour masquer la barre), « Alerter un post » et « Mes sanctions », vue catégorie repliable, icône de l'application au choix, badge « +N » des drapeaux aligné sur le web, sondage visible après le vote, correctifs (citations, navigation, super favoris par compte). Détail par version dans [`app/CHANGELOG.md`](https://github.com/ForumHFR/redface2/blob/main/app/CHANGELOG.md).
- **Canal dev** : aligné sur la bêta (0.55.0) au 8 septembre 2026 ; il reçoit les releases dev suivantes avant leur promotion en bêta.

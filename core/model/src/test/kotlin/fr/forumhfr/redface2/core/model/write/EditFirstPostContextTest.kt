...
/**
 * Pins the `EditFirstPostContext.init` subcat contract (#213). Mirrors
 * `ReplyContext` / `EditPostContext` : `subcat = 0` (a category without
 * sub-category) is postable, only the SUBCAT_UNKNOWN sentinel (-1) is rejected.
 * The FP edit always lives on page 1.
 */
class EditFirstPostContextTest {

    @Test
    fun `subcat zero is accepted as a category without sub-category`() {
        val context = EditFirstPostContext(cat = 32, subcat = 0, topicId = 1, page = 1, numreponse = 5)

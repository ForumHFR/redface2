...
const val MAX_ZOOM_SCALE = 3f
const val ZOOM_RELEASE_SNAP_THRESHOLD = 1.03f

/** Maximum displayed overshoot past [MAX_ZOOM_SCALE] while the fingers keep squeezing. */
const val ZOOM_RUBBER_BAND_RANGE = 0.25f
 ...
 *
 * The floor is hard: pinching below 1× never shrinks the reader. Up to [MAX_ZOOM_SCALE], the raw
 ...
 * restores the [MAX_ZOOM_SCALE] ceiling when the fingers lift.
 */
fun clampScaleDuringPinch(raw: Float): Float {
 ...
 * return to the hard [MAX_ZOOM_SCALE] ceiling. Values between those thresholds are preserved.
 */
fun resolveScaleOnRelease(gestureScale: Float): Float {
 ...
 */
fun panXRange(scale: Float, widthPx: Float): ClosedFloatingPointRange<Float> {
 ...
 */
data class VerticalDistribution(
    val panYNew: Float,
 ...
 */
fun upwardListRequestViewportPx(
    deltaScreenPx: Float,
    scale: Float,
): Float {
 ...
 * reader never exposes space outside the list width. Positive [deltaX] moves the layer right.
 */
fun panStep(
    panXOld: Float,
    deltaX: Float,

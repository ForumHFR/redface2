...
class EditorLayoutBudgetTest {
    ...
    fun `regular window reserves the requested field minimum plus spacing`() {
        ...
        assertEquals(188.dp, editorControlsMaxHeight(available = 360.dp, fieldMin = 160.dp))
    }
    ...
    fun `short window gives controls forty percent instead of collapsing either zone`() {
        ...
        assertEquals(96.dp, editorControlsMaxHeight(available = 240.dp, fieldMin = 160.dp))
    }

    @Test
    fun `tiny and invalid windows remain bounded`() {
        assertEquals(40.dp, editorControlsMaxHeight(available = 100.dp, fieldMin = 160.dp))
        assertEquals(0.dp, editorControlsMaxHeight(available = (-1).dp, fieldMin = 160.dp))
    }
}

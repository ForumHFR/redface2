...
class HfrApiClientTest {
    ...
    @Test
    fun `getFlagTopics hits global topics URI with bucket segment + auth`() = runTest {
        server.enqueue(MockResponse().setResponseCode(200).setBody("{}"))

        client.getFlagTopics(
        ...
        val url = requireNotNull(recorded.requestUrl)
        assertEquals("/webservices/rest_api.php", url.encodedPath)
        assertEquals("forums/hardwarefr/topics/participated/", url.queryParameter("uri"))
        assertEquals("1", url.queryParameter("page"))
        assertEquals("50", url.queryParameter("results_per_page"))
    }

    @Test
    fun `getFlagTopics READ bucket maps to topics read segment`() = runTest {
        server.enqueue(MockResponse().setResponseCode(200).setBody("{}"))

        client.getFlagTopics(bucket = HfrRestFlagBucket.READ)

        val recorded = server.takeRequest()
        val url = requireNotNull(recorded.requestUrl)
        assertEquals("forums/hardwarefr/topics/read/", url.queryParameter("uri"))
    }

    @Test
    fun `getFlagTopics FAVORITES bucket maps to topics favorites segment`() = runTest {
        server.enqueue(MockResponse().setResponseCode(200).setBody("{}"))

        client.getFlagTopics(bucket = HfrRestFlagBucket.FAVORITES)

        val recorded = server.takeRequest()
        val url = requireNotNull(recorded.requestUrl)
        assertEquals("forums/hardwarefr/topics/favorites/", url.queryParameter("uri"))
    }
    ...
    @Test
    fun `getFlagTopics rejects page = 0 and out-of-range resultsPerPage`() = runTest {
        val zeroPage = runCatching {
            client.getFlagTopics(bucket = HfrRestFlagBucket.PARTICIPATED, page = 0)
        }.exceptionOrNull()
        assertTrue("expected IAE for page=0, got $zeroPage", zeroPage is IllegalArgumentException)

        val tooLarge = runCatching {
            client.getFlagTopics(bucket = HfrRestFlagBucket.PARTICIPATED, resultsPerPage = 101)
        }.exceptionOrNull()
        assertTrue("expected IAE for rpp=101, got $tooLarge", tooLarge is IllegalArgumentException)
    }

    @Test
    fun `getFlagTopics surfaces SessionExpiredException on a login redirect`() = runTest {
        ...
        val error = runCatching { client.getFlagTopics(bucket = HfrRestFlagBucket.PARTICIPATED) }
            .exceptionOrNull()
